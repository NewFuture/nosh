...
impl Agent {
    ...
    fn exec_call(
        ...
        match call.name.as_str() {
            ...
            "read_file" | "list_dir" | "search" => self.read_tool(shell, call, approval, ui),
    ...
    fn read_tool(
        ...
        let r = match call.name.as_str() {
            "read_file" => tools::read_file(call, &cwd),
            "list_dir" => tools::list_dir(call, &cwd),
            _ => tools::search(call, &cwd, |child| {
                if !root_protected
                    && child != path
                    && let Err(e) = self.approve_read(child, &ctx, call, approval, ui)
                {
                    denied = Some(e);
                    return Err("reading a protected search path was denied".into());
                }
                Ok(())
            }),
        };
        if let Some(e) = denied {
            ui.tool_end("protected search path denied");
            return e;
        }
        match r {
            Ok(t) => {
