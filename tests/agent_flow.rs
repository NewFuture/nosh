...
fn multi_step_task_uses_tool_results() {
    ...
    assert_eq!(names, ["run_command", "read_file", "list_dir", "search"]);
    assert!(
        ui.events
            .iter()
...
#[test]
fn search_checks_protected_roots_dotdot_symlinks_and_descendants() {
    let _g = setup();
    let dir = tmpdir("search-protected");
    ...
    for path in ["public/../private/secret", "link", "."] {
        let engine = MockChatEngine::new(vec![
            vec![call("search", json!({"pattern": "needle", "path": path}))],
            vec![text("Denied.")],
        ]);
        let received = engine.received();
        let mut a = Agent::new(
        ...
        a.run_task(
            &mut sh,
            TaskInput::new(Trigger::Pipe, "search"),
            &mut approval,
            &mut RecordUi::default(),
        );
        assert_eq!(approval.seen.len(), 1, "{path}");
        assert_eq!(approval.seen[0].tool, "search");
        let results = tool_results(&received.lock().unwrap());
