...
> 对应 `docs/MVP-PLAN.md`（T0–T7）与 `docs/DESIGN.md` v0.4；§5.3 的内存优化按设计 v0.6（PR #2）§2.3 与 §16 #12 实施，审批确认策略和脱敏按设计 v0.8 的 §16 #14、#15 调整（§2.2、§2.3）。多平台 CI（issue #7，设计 v0.11）的结果和随之修复的平台问题见 §2.5。固定 seed 的原生 main 基线（issue #3，设计 v0.13）另见 §3.1，不替换早期手工数据。本报告记录实现范围、偏离设计之处、10 个端到端场景的结果、性能数据、已知问题和对 M2 的建议。

## 1. 结论

历史 MVP 的工具清单和测试数量保留原始口径；当前 v0.14 已移除模型建议工具，建议模式直接输出 program，终端交接由 harness 完成，新增内置 `search`。下文旧工具名只记录历史行为，不是当前接口。
