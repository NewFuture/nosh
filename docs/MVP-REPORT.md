...
- 10 个真实模型场景各跑 3 次（共 30 次）：27 次完全正确，另有 2 次结论正确但回答里有小错（总数算错、前后数字矛盾），1 次失败（2B 模型用 `sort` 排错了字段）。
- 性能（WSL2，Xeon 8370C 8 核，Q4_K_M，release 构建）：decode 19–25 tok/s，prefill 102–147 tok/s，热对话首 token 0.4–1.0 s，无 rc 启动到提示符 6–7 ms。
...
测试：`cargo test --workspace` 共 124 个测试（权限的 247 条用例按表驱动放在少数几个测试函数里），其中 `tests/agent_flow.rs` 用 MockChatEngine 覆盖多步任务、审批与拒绝理由、Dangerous 强确认与编辑后重新评估、`exec`/`exit` 拦截、错误回灌与放弃、截断与落盘、步数上限、无 TTY、`propose_command`、只读工具与受保护路径、超时，以及 REPL + agent 联动（`#`、`gti status`、agent `cd` 后用户 `pwd`、中文 not_found）。另有 5 个 `#[ignore]` 测试：3 个需要真实模型（本地已通过），1 个注意力基准，1 个权限诊断输出。CI（ubuntu-latest：fmt、clippy -D warnings、test）每次提交都是绿色。
...
**方法**：交互场景用 Python pty 驱动真实的 `nosh --norc`（发送按键、按提示回答审批），CLI 场景直接调用 `nosh -a`/`nosh -s`。夹具在 `~/nosh-e2e`（不同大小的文件、一个多语言小项目、待重命名的 `.txt`、用本仓库 git bundle 克隆的仓库、`python3 -m http.server 8080` 监听端口）。所有场景用最终构建连续跑 3 轮，每轮重建夹具；`NOSH_STATS=1` 记录每个任务的 token 数、速度、首 token 延迟和 RSS。

| # | 场景 | 输入 | 3 轮结果 | 步数 | 任务耗时 |
|---|---|---|---|---|---|
| 1 | 查找大文件 | `# find the 3 largest files under this directory` | ✔ ✘ ✔ | 4 / 4 / 4 | 24.9 / 28.3 / 23.5 s（首个任务，含冷 prefill） |
| 2 | 端口占用 | `# which process is listening on port 8080?` | ✔ ✔ ✔ | 2 / 2 / 2 | 5.5 / 6.5 / 6.7 s |
| 3 | 统计代码行数 | `# count the lines of code in this project, grouped by language` | ✔ ✔ ✔ ¹ | 4 / 4 / 4 | 28.3 / 29.1 / 29.2 s |
| 4 | 批量重命名（需审批） | `# rename every .txt file in this directory to .md` | ✔ ✔ ✔ | 4 / 3 / 3 | 16.1 / 17.0 / 16.3 s |
| 5 | 中文提问 | `# 这个项目里有哪些 Python 文件？` | ✔ ² ✔ ✔ | 2 / 3 / 2 | 6.4 / 5.3 / 9.6 s |
| 6 | 拼写纠错 | `gti status`、`pyhton3 --version` | ✔ ✔ ✔ | 0（本地） | 4 ms（Linux PATH）/ 77 ms（WSL PATH） |
| 7 | 执行失败后分析原因 | `python3 broken.py`（exit 1）后输入 `#` | ✔ ✔ ✔ | 2 / 2 / 2 | 19.2 / 19.7 / 13.7 s |
| 8 | 管道总结 git log | `git log --stat -8 \| nosh -a "summarize …"` | ✔ ✔ ✔ | 1 / 1 / 1 | 27.5 / 32.9 / 36.4 s（含加载模型，2.5K token 冷 prefill） |
| 9 | `nosh -s` 生成命令 | `nosh -s "compress the logs directory into logs.tar.gz"` | ✔ ✔ ✔ | 1 | ≈ 10.8 s（含加载模型） |
| 10 | agent `cd` 后状态延续 | `# change the current directory to data and list the files there`，然后用户执行 `pwd` | ✔ ✔ ✔ | 2 / 2 / 2 | 7.5 / 9.6 / 6.4 s |

¹ 第 1 轮各语言行数都对（Python 18、JS 5、Shell 3、Rust 3），但正文把总数写成 31（应为 29）。
² 第 1 轮第一句说"2 个 Python 文件"，随后列出 3 个并总结"共 3 个"。

**逐项说明**：

1. **查找大文件**：成功的两轮用 `find … | sort -h | head` 得到正确的前三（20 MB、12 MB、4.8 MB）。失败的一轮生成了 `find … -exec stat --format='%n %s' {} \; | sort -rn`，按文件名而不是大小排序，结论错误。
2. **端口占用**：三轮都用 `ss -tlnp | grep 8080` 找到 `python3` 及其 PID（均为 Safe，自动执行）。
3. **统计代码行数**：模型先用 `list_dir` 浏览，再用 `read_file` 读每个文件并计数，结果正确但步数偏多（4 步约 29 s），没有用 `wc -l`。
4. **批量重命名**：生成 `for f in *.txt; do mv "$f" "${f%.txt}.md"; done`，因为写入目标是运行时计算的路径被评为 **DANGEROUS**，审批卡片要求键入 `yes`；执行后 4 个文件改名，`readme.md` 保持不变。
5. **中文提问**：三轮都用中文回答，列出正确的 3 个文件。
6. **拼写纠错**：`gti status` → 输入行预填 `git status`，`pyhton3 --version` → `python3 --version`，都不自动执行，也不调用模型。
7. **失败分析**：`python3 broken.py` 失败后提示 `✗ exit 1 · Ctrl+G or # to ask AI`；输入 `#` 后，agent 读取脚本，指出缺少 `config.json`，并给出修复方法。第 1 轮还请求重跑脚本以确认错误（Mutating，经审批后执行）。
8. **管道总结**：stdin 截断后作为附件（只读工具），三轮都准确概括了本仓库最近的提交（CLI、core、shell、permissions、llm、hub、CI）。
9. **`nosh -s`**：stdout 恰好一行 `cd logs && tar -czvf ../logs.tar.gz .`（`bash -n` 语法检查通过），说明写到 stderr，退出码 0。
10. **状态延续**：agent 执行 `cd …/data && ls -la`，界面提示 `cwd → …/data`；随后用户输入的 `pwd` 输出 `/home/newfuture/nosh-e2e/big/data`。

**调优记录**（最终 3 轮之前的试跑中发现，已修复）：
...
| 5 | 在共享会话中完成多步任务：Safe 自动执行，Mutating/Dangerous 先审批；agent `cd` 后状态保留 | ✔ 场景 1–4、7、10；审批流程有集成测试 |
...
release 构建，8 个推理线程（物理核心数），Q4_K_M，上下文 8K：

| 指标 | 目标 | 实测 | |
|---|---|---|---|
| decode | ≥ 12 tok/s | 24.6–25.7 tok/s（短上下文）；19–23 tok/s（场景中，1.2–2.9K 上下文）；16.4 tok/s（4.4K 上下文） | ✔ |
| prefill | ≥ 100 tok/s | 131–147 tok/s（短增量）；128–143 tok/s（场景首个任务约 1K token）；140 tok/s（2.1K token 冷 prompt）；102 tok/s（2.2K→4.3K 上下文） | ✔ |
| engine 常驻内存（8K） | ≤ 2.3 GB | 3.2–3.8 GB（RSS 峰值） | ✘ 见 §6 |
| 启动到提示符（不含用户 rc） | ≤ 50 ms | 6–7 ms（加载 Ubuntu 默认 `~/.bashrc` 时 55 ms） | ✔ |
| `nosh -c` 相对 bash 的额外开销 | ≤ 10 ms | `nosh -c true` 3.1 ms/次，`bash -c true` 4.7 ms/次 | ✔ |
| 命令不存在 → 本地拼写建议 | ≤ 50 ms | 4 ms；WSL 默认 PATH（42 项中 32 项在 `/mnt/c`，经 9p 访问）下为 77 ms | ✔（WSL 下 ✘） |
| `#` 任务首 token（engine 已加载、同一对话） | ≤ 0.8 s | 0.42–0.97 s，中位数 0.76 s | ✔ |
| `#` 任务首 token（engine 冷启动） | ≤ 3 s（依赖磁盘前缀缓存） | 6.1–7.0 s（另加模型加载 2–3 s） | ✘ 需 M2 |
...
3. **2B 模型的可靠性**：偶尔拼错命令（场景 1 按文件名排序）、算错数字、自相矛盾；倾向于用 `list_dir`/`read_file` 逐个查看，而不是一条 `find`/`wc` 命令，因此步数偏多。temperature 1.0（官方推荐的设计默认值）放大了结果的波动。
4. **brush 后台作业**：`cmd &` 显示 `[1]+ <pid unknown>`，`kill %1` 失败。brush-core 0.5 把异步命令当作任务运行，没有 pid；需要向上游修复。
5. **agent 命令被中断时丢弃 brush 的 future**：超时或 Ctrl-C 时，向新进程组发送 SIGINT/SIGTERM（2 s 后 SIGKILL）并放弃整条命令行，行为与 bash 中按 Ctrl-C 一致。如果当时正处在 shell 函数内部，环境作用域会被修复，但 brush 的调用栈帧可能残留（很少见，需要上游提供取消接口）。
6. **WSL 特有**：PATH 中的 `/mnt/c` 目录经 9p 访问很慢，"命令不存在"的判定约 77 ms；首次列 PATH 约 0.4 s（在后台预热）。
7. **Ctrl+G 建议使用独立会话**：`LocalChatEngine` 只维护一份 KV，切换会话时从最长公共前缀开始重算，所以 Ctrl+G 之后的下一个 `#` 任务要重新 prefill 主对话。
8. **保守的风险分级**：写入目标是运行时计算的路径（`for f in *.txt; do mv …`、`find -exec cp {} {}.bak`）一律评为 Dangerous，需要键入 `yes`。
9. **审批卡片出现时会丢弃预输入**：为防止之前缓冲的按键误答审批，出现卡片时清空终端输入缓冲；设计文档里写的是"agent 运行期间用户的输入先缓冲"。
10. **尚未实现（MVP 范围外或加分项）**：后台下载、多源并行下载、`ai history/private/undo/model`、`thinking = "auto"`、`engine.*` 配置（MVP 在进程内推理，这些键会被接受但忽略）。
...
| agent 超时 | 中断 | 中断整条命令行（SIGTERM，2 s 后 SIGKILL），不继续执行后续命令 | brush 没有取消接口；与 bash 按 Ctrl-C 的行为一致 |
...
3. **提高 2B 模型的可靠性**：约束解码（工具名、参数名）；在 system prompt 里给出"计数、搜索用 run_command + 标准工具"之类的简短提示并做 A/B；agent 模式试用较低的 temperature（例如 0.6–0.7），与官方推荐值 1.0 对比成功率；对常见任务（大文件、端口、行数）做评测集回归。
4. **多会话 KV**：按会话保留 KV（或至少为建议模式单独保留一份），避免 Ctrl+G 冲掉主对话的缓存。
5. **brush 上游**：异步作业的 pid 与 `kill %n`；可取消的执行接口（agent 超时时不必丢弃 future）；进程创建钩子（为 Landlock/seccomp 沙箱做准备）。
