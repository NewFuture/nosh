...
> **状态：已完成，作为历史计划保留。** 本文对应设计 v0.4 的精简 M0 + M1，在 **Linux** 上跑通 AI shell、本地推理、自动下载和权限审批；勾选状态、环境、估时和 API 线索记录的是当时的实施过程，不是当前待办或完整 API 契约。
>
> 交付与偏差见 [MVP 报告](MVP-REPORT.md)；当前能力见 [设计文档 §0.3](DESIGN.md#03-实现状态)，后续工作见 [§14 里程碑](DESIGN.md#14-里程碑)，实际接口见 [§3.4](DESIGN.md#34-核心接口与代码入口)。
>
> 当时的工作方式：功能分支开发、小步提交，完成后开 PR 到 `main`；需要人工决策时采用当时设计的默认值并在 PR 中记录。

**导航**：[范围](#1-mvp-范围) · [环境](#2-环境) · [工程结构](#3-工程结构) · [任务分解](#4-任务分解) · [验收标准](#5-验收标准) · [风险](#6-风险与应对)
