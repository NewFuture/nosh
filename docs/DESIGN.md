...
- **放行边界**：参数中只要有选项、已存在的路径或绝对路径，就继续按命令执行，例如 `rm -rf all temp files`。按实际参数计数，`rm 'all temp files'` 中的一个引号参数不会拆成 3 个普通单词。
- **处理方式**：执行前先提示 `看起来像自然语言：↵ 交给 AI / Ctrl+E 仍按命令执行`。
- **为什么需要**：例如 `rm all temp files` 可能是在描述任务，而直接执行会把这些词逐个当作文件名。这是轻量的歧义检查，不代替命令风险分级。
- 这个检查完全在本地完成，耗时在微秒级。
...
| AI 触发 | 400 条标注语料：合法命令 200、中文自然语言 60、英文自然语言 50、拼写错误 50、安全网输入 40 | 所有样本逐条匹配期望动作，纠错需匹配完整命令；安全网误拦截 < 0.5%；中文自然语言 100% 交给 AI；破坏性命令误执行次数为 0；纠错命中率 ≥ 90% |
...
AI 触发的[语料](../crates/nosh-shell/tests/fixtures/trigger_corpus.jsonl)采用 JSONL，每条记录包含 `id`、`category`、`input`、`expected`，纠错样本还需填写完整的 `corrected`；可选 `reason` 只解释标注，不豁免失败。[测试驱动](../crates/nosh-shell/tests/trigger_corpus.rs)直接调用本地规则 `classify`：PATH 只含临时假可执行文件，工作目录及文件树受控，不加载用户 rc、不调用模型、不执行语料命令，并在两个独立临时目录中核对结果一致。

统计按显式分组计算：中文路由率以中文自然语言组为分母，合法命令里的中文参数不计入；安全网误拦率以合法命令组为分母；破坏性误执行只统计应触发安全网组中的 `Execute`，不把 §4.2 规定放行的命令算作误执行；纠错命中率以拼写错误组为分母，只有完整修正结果正确才算命中。即使四项总体指标达标，任何一条不匹配仍会使 CI 失败，不设跳过或预期失败项。CI 的 workspace 测试使用 `--show-output`，通过时也会输出规模、分组计数及四项指标。单独运行：

```sh
cargo test -p nosh-shell --test trigger_corpus --locked -- --show-output
```
...
| v0.10 | AI 触发判定语料（issue #5）：§13.2 的 AI 触发从"至少 1,000 条"改为实际的 400 条标注语料（合法命令 200、中文自然语言 60、英文自然语言 50、拼写错误 50、安全网输入 40），PATH 只含临时假可执行文件，每条严格匹配，并断言四项指标，CI 通过时也输出指标；§4.2 命令名不存在时先识别常见英语问句（如 `can you …`、`why is …`），不再误纠成 `cat`、`who`；写明安全网的放行边界（带选项、已存在路径或绝对路径时照常执行，如 `rm -rf all temp files`），并删去"同名文件存在时会被误删"这条与放行规则矛盾的理由 |
