...
> **代号**：nosh（Native Offline SHell，可以改名）　**版本**：v0.12　**日期**：2026-09-25　**默认模型**：MiniCPM5-2B（Apache-2.0）
>
> nosh 是一个兼容 Bash、内置本地小模型、可以断网运行的 AI shell。v0.5–v0.8 根据 MVP 的实测结果和代码审查，补充了内存模型、性能数据和实现要点；v0.9 加入日常开发命令基准，v0.10 加入 AI 触发判定语料，v0.11 补充 aarch64 和 macOS 上的平台细节，v0.12 将固定 seed 的真实模型评测集和基线口径入库。修订记录见附录 E，产品决策见 §16。
>
...
- **AI 输出块**：AI 的输出以带左侧竖线的块插入回滚区。
  ...
  - 用户自己的命令按原样输出。
- **字符与终端降级**：
  - stdout/stderr 独立判断能力；`NO_COLOR`（非空）或 `CLICOLOR=0` 禁用样式，`TERM=dumb`、`unknown` 或缺失时禁用 ANSI 动画和高级行编辑，改用基本输入编辑。非 UTF-8 locale 和旧控制台使用 ASCII 装饰，不转码用户数据。
...
| Agent 评测 | [MVP 10 场景](../eval/README.md)，每场景默认 5 个固定 seed，每轮 50 次独立进程试验（含 5 次不加载模型的本地纠错）；夹具每次重建 | 自动判定回答事实与最终状态；记录通过率、步数、确认次数、首 token 延迟、总耗时及 RSS 峰值，输出 JSON/Markdown 并按场景/seed 比较版本。双跑检查判定和最终状态一致，回答、输入及工具轨迹差异另报；不进入每 PR 的真实模型 CI |
