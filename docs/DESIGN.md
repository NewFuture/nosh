...
> **代号**：nosh（Native Offline SHell，可以改名）　**版本**：v0.6　**日期**：2026-09-24　**默认模型**：MiniCPM5-2B（Apache-2.0）
>
> nosh 是一个兼容 Bash、内置本地小模型、可以断网运行的 AI shell。v0.5 和 v0.6 根据 MVP 的实测结果，补充了内存模型、性能数据和实现要点。修订记录见附录 E，产品决策见 §16。
>
...
| G8 | 低资源 | 8K 上下文时，推理进程约 2.9 GB（目标 ≤ 3.0 GB；MVP 实测 3.2–3.8 GB，M2 优化，见 §2.3）；纯 CPU 下目标速度 ≥ 12 tok/s；多个终端共用一份模型 |
...
| **目标（M2，已决策）**：加载时提前重排 Q4K 并释放其原始权重，KV 用 f16，8K 上下文 | **约 2.9 GB（目标 ≤ 3.0 GB）** | 只释放 Q4K 的原始权重（约 0.9 GB）。Q6K 的原始权重要留给 decode 使用，速度与现状相同 |
...
- **candle**（固定到 main 分支的某个 git rev）：
  ...
  - 重排布局是 candle 懒加载的缓存，原始权重会一直保留（见 §2.3）。nosh 的做法：加载时提前重排 Q4K 权重，并释放它的原始数据；Q6K 的原始权重要留给 decode 使用。这需要给锁定的 candle 版本打一个小补丁（vendor），同时向上游提议增加开关。
...
| 6 | 重排完成后，原始权重仍然常驻内存 | 加载时提前重排 Q4K 权重并释放其原始数据，约省 0.9 GB；Q6K 保留原始权重给 decode 用；embedding 和 output 不重排（M2） |
...
| engine 常驻内存（8K） | ≤ 3.0 GB（约 2.9 GB，见 §2.3） | 3.2–3.8 GB，原因和改进方法见 §2.3 |
...
| candle 的重排布局与原始权重同时常驻（Q4K 重排后膨胀约 1.33 倍，Q6K 约 1.52 倍） | 加载时提前重排 Q4K，并释放其原始权重（patch 锁定的 candle 版本）；向上游提议"提前重排并释放原始权重"的开关（见 §2.3） |
