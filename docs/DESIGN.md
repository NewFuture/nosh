...
5. **效果未知的命令**：规则表之外的命令、本地路径的可执行文件（如 `./gen.sh`、`/tmp/x`），以及用解释器执行脚本文件或内联代码的命令（如 `bash x.sh`、`python x.py`、`node -e …`），实际做什么无法静态判断。这类命令会被标记为"效果未知"，在 auto 模式下也必须确认（见 §6.3）。
...
| auto | 自动 | 工作区内自动；工作区外或效果未知时确认 | 强确认 | 拒绝 |
| yolo | 自动 | 自动 | 确认 | 拒绝 |
...
| v0.8 | 根据 PR #1 的 Copilot 代码审查补充：§6.2 新增"效果未知的命令"（规则表之外的命令、本地可执行文件、解释器执行的脚本或内联代码），在 auto 模式下也必须确认，§6.3 的审批矩阵同步修改；§8.2 的校验缓存改用纳秒级 mtime 加文件身份 |
