...
| v0.10 | AI 触发判定语料（issue #5）：§13.2 的 AI 触发从"至少 1,000 条"改为实际的 400 条标注语料（合法命令 200、中文自然语言 60、英文自然语言 50、拼写错误 50、安全网输入 40），通过标准增加"每条样本都符合期望、纠错匹配完整命令"；§4.2 命令名不存在时先识别常见英语问句（如 `can you …`、`why is …`），不再误纠成 `cat`、`who`；写明安全网的放行边界（带选项、已存在路径或绝对路径时照常执行，如 `rm -rf all temp files`），并删去"同名文件存在时会被误删"这条与放行规则矛盾的理由 |
