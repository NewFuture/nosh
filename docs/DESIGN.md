...
| G8 | 低资源 | 8K 上下文时，推理进程 ≤ 3.0 GB（已实现：实测 2.69 GiB，约 2.88 GB，见 §2.3）；纯 CPU 下目标速度 ≥ 12 tok/s；多个终端共用一份模型 |
...
**原则：方便优先。** 只在真正危险、不可逆，或者会越出工作区的操作上打断用户。能通过分析确认安全的就不提示；宁可在少数边界情况下放宽一些，也不要频繁弹出确认，更不要让用户反复键入 `yes`（§16 #14）。
...
5. **效果未知的命令**：规则表之外的命令、本地路径的可执行文件（如 `./gen.sh`、`/tmp/x`），以及用解释器执行脚本文件或内联代码的命令（如 `bash x.sh`、`python x.py`、`node -e …`），实际做什么无法静态判断。
   - 这类命令按 Mutating 处理，不额外提示：auto 模式下照常自动执行，confirm 模式下单键确认。
...
- **运行时才确定的写入目标**（例如 `for f in *.txt; do mv …`、`find . -exec cp {} …`）：
  ...
  - MVP 原先一律按 Dangerous 处理，属于过度提示，已按方便优先的原则放宽。
...
- **可靠性**：
  ...
  - 已校验的文件在 manifest 中记录纳秒级 mtime、大小和文件身份（Unix 上为 inode/dev 和 ctime），任何一项变化都要重新校验；
  - 用文件锁防止并发下载；
  - 下载前检查磁盘空间；
  - 失败时按指数退避重试。
...
| `#` 任务的首个 token（engine 冷启动、磁盘缓存命中） | ≤ 3 s | 场景中 4.7–5.5 s（内存优化前为 6.2–7.0 s），另外还有 1.8–2.1 s 的模型加载；短 prompt 冷启动的首个 token 为 0.72–0.86 s。还需要磁盘前缀缓存和常驻 engine（M2） |
| 审批卡片出现 | ≤ 50 ms | 没有单独测量 |
...
| 推理正确性 | 模板对比 HF `apply_chat_template`；logits 对比参考实现（llama.cpp，或者 KV 用 f32 的自身实现），用真实 prompt 加 teacher forcing | 模板逐字节一致。logits 用分布类指标衡量：高置信的 top-1 一致；平均 KL 不明显超过"f32 KV 只改动 1 个最低位"的扰动对照（f16 KV 实测 0.0106，对照为 0.0110）；真实后续 token 的 NLL 不变差；余弦中位数 > 0.995；报告 top-5 重合度。不使用"余弦 > 0.999"这条标准（见 §16 #13） |
...
| **M1 本地版 MVP**（✔ 已完成，见 PR #1 与 `docs/MVP-REPORT.md`） | 6 周 | **nosh shell**：AI 触发、安全网、本地纠错、登录 shell 兼容；终端与信号模型；故障隔离。**harness**：任务头。**权限** v1。**工具**：`run_command`、`read_file`、`list_dir`、`propose_command`。**推理**：fork 改造 #1–#3 和 #5、模板、采样、对话内前缀复用、资源自适应。**下载与离线导入**。**CLI**：`-a`、`-s`、管道。**平台**：Linux 与 macOS。**内存优化**（追加）：加载时重排 Q4K 并释放其原始权重，KV 改为 f16，8K 上下文实测 2.69 GiB |
...
| 14 | 权限提示策略 | 方便优先，避免过度提示，也不要过严。效果未知的命令按 Mutating 处理，auto 模式下照常自动执行，shell 脚本通过分析内容把关，不额外提示；运行时才确定、但在工作区内的写入目标按 Mutating 处理；Dangerous 仍然需要确认，Forbidden 仍然拒绝（§6） |
...
| v0.8 | 根据 PR #1 的 Copilot 代码审查，以及用户"方便优先"的要求（§16 #14）：§6 开头增加"方便优先"原则；§6.2 新增"效果未知的命令"，按 Mutating 处理、不额外提示，shell 脚本会分析其内容；工作区内运行时才确定的写入目标，从 Dangerous 降为 Mutating；§8.2 的校验缓存改用纳秒级 mtime 加文件身份 |
