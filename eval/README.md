...
Hosted 工作流显式使用 **每次试验 60 秒**期限，本地默认仍为 **240 秒**。250 个试验即使全部耗尽期限，试验时间也为 250 分钟，六小时 job 留有 110 分钟用于构建、夹具、检查点和上传；构建前还会按实际场景、seed、重复次数检查预算，至少预留 90 分钟非试验时间，超出直接报错。准备阶段另设超时，不把正常的最坏评测计划留给 job 强制截断。
...
[`scenarios.json`](scenarios.json) 是唯一的场景配置。`schema_version: 2` 声明输入、夹具、审批、判定器、`completions` 和 `expect`；仍支持 v1 场景及旧报告。当前 **`dataset_revision: 2`** 单独标记任务与裁判语义；未声明此字段的历史场景/报告按 revision 1 解释，不改写旧数据。REPL 输入原样发送，失败求助先观察真实退出码和诊断，不插入会覆盖“上次失败命令”的探针。
...
以下均是 **revision 1 的历史基线**。revision 2 尚未运行真实模型，也没有用新规则重评或替换旧记录；48.0% 不是当前新评测语义下的通过率。
